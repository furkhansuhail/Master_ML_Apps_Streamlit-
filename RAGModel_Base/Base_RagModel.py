...
class RagModel:
    ...
    def SemanticSearch(self):
        while True:
            query = input(
                "Enter your query for Semantic Search (type 'LLM' to move to next Function or exit to quit): ").strip()
            if query.lower() == "llm":
                self.LocalLLM()
    ...
    def LocalLLM(self):
        ...
        self.llm_model = AutoModelForCausalLM.from_pretrained(pretrained_model_name_or_path=model_id,
                                                              ...
                                                              attn_implementation=attn_implementation, token = hf_token)  # which attention version to use
